...
data Command = Open Position | Mark Position | Unmark Position
...
generateBombPositions size numBombs = replicateM numBombs (generatePosition size)
...
getCommand = do
  ...
  commandStr <- getLine
  case parseCommand commandStr of
...
executeCommand :: GameState -> Command -> GameState
executeCommand gameState command =
  case command of
    Open pos -> openCell pos gameState
    Mark pos -> markCell pos gameState
    Unmark pos -> unmarkCell pos gameState
...
openCell pos gameState =
  if isValidPosition gameState pos && not (isOpened pos gameState)
...
markCell pos gameState =
  if isValidPosition gameState pos && not (isOpened pos gameState) && not (isMarked pos gameState) && length (marked gameState) < length (bombs gameState)
...
unmarkCell pos gameState =
  if isValidPosition gameState pos && isMarked pos gameState
    then gameState { marked = filter (/= pos) (marked gameState) }
    else gameState

-- Verifica se uma posição é válida
isValidPosition :: GameState -> Position -> Bool
isValidPosition gameState (row, col) = row >= 0 && row < size && col >= 0 && col < size
  where
